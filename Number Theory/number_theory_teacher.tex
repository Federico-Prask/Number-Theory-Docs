...
\usepackage{hyperref}
\hypersetup{colorlinks=true,linkcolor=MidnightBlue,urlcolor=MidnightBlue,bookmarksnumbered=true,pdfauthor={Federico Prask},pdftitle={数论·教师版}}
...
\newtcolorbox{tip}[1][先解释一个词]{enhanced,breakable,colback=tipbase!4,colframe=tipbase!60,boxrule=0.4pt,arc=2mm,title={#1},fonttitle=\bfseries,colbacktitle=tipbase,coltitle=white,left=2mm,right=2mm,top=1mm,bottom=1mm,before skip=6pt,after skip=6pt}
...
\vspace{1cm}
{\fontsize{46}{65}\selectfont\bfseries\color{ink}数论}\par
\vspace{0.5cm}
...
% \section*{本次校订的重点}
% 已重写 exgcd 的整除条件与缩放、数学游戏的平方 gcd、CRT 构造式、欧拉反演中的错误等式、LTE 的指数笔误、广义逆元与 BSGS 的条件、杜教筛及 PN 的复杂度表达。还特别修正了 CF516E 的错误“最小起点支配全环”结论，以及 $m=n\varphi(n)$ 题解中把 $-1$ 当成非平凡平方根的错误。完整勘误表在附录。
...
\chapter{预备：把书里的词、符号和说话方式先说清楚}
\begin{tip}[这一章解决什么问题]
后面的章节默认你会说一种“数学方言”：约数、模、可逆、赋值、卷积、双射、记忆化、阈值……这些词每一个似乎都能用一句话解释清楚，但正文往往不会专门说。本章把全书会用到、而高中课本不一定会说的词与记号一次交代清楚，并在末尾给出一张“高中叫法 $\leftrightarrow$ 本书叫法”的对照表和一段“不跳步”的证明示范。

本章不教新数学。它的作用很简单：以后正文里哪个词把你卡住了，就回到这里查它到底是什么意思。
\end{tip}
